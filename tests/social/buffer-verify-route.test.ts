...
describe("Buffer LinkedIn verification route", () => {
  ...
  it("returns only non-secret channel metadata", async () => {
    ...
    expect(await response.json()).toEqual({ transport: "buffer", channel: { id: "channel-1", name: "Miloosh", displayName: "Miloosh", descriptor: "LinkedIn Page", service: "linkedin", type: "page", isDisconnected: false, isLocked: false } });
  });
});
